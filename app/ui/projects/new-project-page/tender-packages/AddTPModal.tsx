...
// AddClientViewModal and AddTPModal can be both refactored to AddModel with the record creating funciton as 
// an argument

import {useState, Dispatch, SetStateAction} from 'react';
import {createTPModel} from '@/app/lib/AutodeskViewer/actions-viewer';
...
export default function AddTPModal({id, setModal, session}: {id: string; setModal: Dispatch<SetStateAction<boolean>>; session: Session | null}) {

    const [uploadStatus, setUploadStatus] = useState('empty');
    const user = session?.user?.name ?? "undefined"
    const createCV = createTPModel.bind(null, id, user)
